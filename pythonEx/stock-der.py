...
#f = io.StringIO(response.text)
#dfs = pd.read_html(f)
dfs = pd.read_csv('mystock.csv',nrows=25)
print(dfs)
world_index = dfs
...
#    price['Adj Close']['2015':].plot(label=name)
    if price.index.is_unique:
         adjClose[name]=price['Adj Close']['2019':]
         print("="+name)
    else :
         print("x:"+name)
...
print(adjClose_pct)
#sns.jointplot("2330TW","S&P 500",adjClose_pct, kind="scatter")
...
#stockinfo['2886TW']['Adj Close']['2019':].plot(label='2886')
#########################################################
# taiwan currency
#
#
tstr=""
#tder=twder.specify_month('USD',2019,8)
tder=twder.past_six_month('USD')
tder.insert(0,('Date', 'B1',	'Sell',	'B2',	'Adj Close'));
for row in tder:
    print(row)
    for i in row:
        tstr=tstr+','+i
...
with open('USD.csv', 'w') as f:    # skip the write csv to save time
    f.writelines(tstr)
    
pder = pd.read_csv('USD.csv', index_col='Date', parse_dates=['Date'] )

print(pder)
#pder['Adj Close'].plot(secondary_y=True)
#stockinfo['2886TW']['Adj Close']['2019':].plot(label='2886')

# check 2886 vs USD
sns.jointplot("2886TW","USD",adjClose_pct, kind="scatter")
